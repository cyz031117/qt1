...
void Check1::on_menu_click1(bool)
{
    ...
    QMessageBox::Button btn = QMessageBox::question(this, "删除Guide", "是否删除该条Guide？");
...
void Check1::on_menu_click2(bool)//重命名
{
    ...
    if(ok&&!str.isEmpty())
    {
        ...
        backDir=myDir;
        myDir.append("./").append(dfname);
        backDir.append("./").append(str).append(".guide");
        QFile fileTemp(myDir);
...
{
    QString dirpath="./message1";//message文件地址
    QStringList filelist = findFiles(dirpath, QStringList() << "*.guide");
    ui->listWidget->clear();
